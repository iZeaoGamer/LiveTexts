    ...
    private static $instance;
   	...
   	public function onDamage(EntityDamageEvent $event){
   		...
   		if($event instanceof EntityDamageByEntityEvent){
   		  ...
   		  if(isset($this->texters[$entity->getId()]) and (!isset($this->removers[$damager->getName()]))){
   			  $event->setCancelled(true);
   		  }elseif(isset($this->removers[$damager->getName()])){
   		  	 $entity->kill();
   		  	 $damager->sendMessage("§6[LiveTexts]§c LiveText removed.");
   		  	 unset($this->removers[$damager->getName()]);
   		  }
   		}
   	}
   				...
   				foreach($entities as $entity){
   					if(isset($entity->namedtag->LiveTextName)){
   				     ...
   				     fclose($dosya);
   						...
   						$entity->setNameTag(strtr($yazi, "ō", " "));
   						$entity->setDataFlag(Entity::DATA_FLAGS, Entity::DATA_FLAG_INVISIBLE, true);
			         $entity->setDataProperty(Entity::DATA_SHOW_NAMETAG, Entity::DATA_TYPE_BYTE, 1);
   					}
   				}
